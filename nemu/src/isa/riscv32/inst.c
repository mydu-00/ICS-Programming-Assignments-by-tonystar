...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , I, if (src1 == src2) s->dnpc = s->pc + imm;);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , I, if (src1 != src2) s->dnpc = s->pc + imm;);
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , I, if ((sword_t)src1 < (sword_t)src2) s->dnpc = s->pc + imm;);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , I, if ((sword_t)src1 >= (sword_t)src2) s->dnpc = s->pc + imm;);
